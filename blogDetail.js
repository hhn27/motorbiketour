...
const contentHeading = document.querySelectorAll('.blog-detail-content-right-detail h2')
const tableContent = document.querySelector('.table-content .table-content-list')
contentHeading.forEach((item, index) => {
    tableContent.innerHTML += `<span class='table-content-item'>${item.innerText}</span>`
})
const tableContentItem = document.querySelectorAll('.table-content .table-content-item')
contentHeading.forEach((item, index) => {
  ...
  var tourWithDes = new Swiper('.letRideSwiper', {
    navigation: {
